...
export function ChatMessages({
 ...
 <div className="mx-auto flex w-full max-w-3xl flex-col gap-5 px-4 py-6">
        {messages.length === 0 && !streamText && (
          <div className="rounded border border-border bg-card px-6 py-10 text-center font-mono">
            {repo.indexStatus === "EXPIRED" ? (
              <>
                <p className="font-medium text-foreground">○ Repository sleeping</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  This repository hasn&apos;t been used recently.
                  <br />
                  Ask a question to wake it up.
                </p>
              </>
            ) : (
              <>
                <p className="font-medium text-foreground">ASK YOUR CODEBASE</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  Ask about architecture, bugs, dependencies, APIs, or implementation details.
                </p>
                <p className="mt-4 text-xs text-muted-foreground">Try:</p>
                <p className="text-xs text-primary/80 mt-1">&quot;Where is authentication handled?&quot;</p>
              </>
            )}
          </div>
        )}
 ...
 return (
 <Message key={message.id} align={isUser ? "end" : "start"} className="animate-in fade-in slide-in-from-bottom-3 duration-300 ease-out">
 <MessageAvatar>
 <Avatar className="size-8 rounded-sm border border-border dark:border-border">
 <AvatarFallback
 className={cn(
 ...
 className={cn(!isUser && "max-w-full", "rounded-none border border-border dark:border-border shadow-none", isUser ? "bg-muted dark:bg-muted text-foreground dark:text-foreground" : "bg-muted/30 dark:bg-background text-foreground/90 dark:text-foreground")}
 >
